...
func _process(_delta):
	if not GlobalVars.get_client_state() == 'loading' and GlobalVars.self_obj != null:
		# Set the items for the current inventory tab
		for item_index in 8:
			var item_node = item_tab_dict[active_inventory_tab][item_index]
			
			if GlobalVars.self_obj.inventory.keys().size() > item_index:
				var player_item = GlobalVars.self_obj.inventory.keys()[item_index]
				...
				if active_inventory_tab.name in ['All', player_item.get_id()['CategoryType']]:
					...
					if GlobalVars.self_obj.inventory[player_item]['equipped']:
		...
		for category in item_categories:
			for item in GlobalVars.self_obj.inventory:
				if item.get_id()['CategoryType'] == category and GlobalVars.self_obj.inventory[item]['equipped']:
...
func item_selected(num):
	...
	for item in GlobalVars.self_obj.inventory:
		...
		if index == num:
			local_item = item
			player_item = GlobalVars.self_obj.inventory[local_item]
...
func update_item_stats(num):
	...
	if GlobalVars.self_obj.inventory.keys().size() > num:
		var hovered_item = GlobalVars.self_obj.inventory.keys()[num]
...
func update_player_stats():
	...
	for stat in GlobalVars.self_obj.stat_dict:
		player_stats_label.text += stat + ' ' + str(GlobalVars.self_obj.stat_dict[stat]) + '\n'
	player_stats_label.text = player_stats_label.text.strip_edges()
	
func option_action(index):
	var server_item_id = GlobalVars.self_obj.inventory[local_item].get('server_id')
	if not server_item_id:
		server_item_id = local_item.get_id()['Instance ID']
	if not GlobalVars.self_obj.in_turn:
		match index:
			0:  # equip item
				var player_inventory = GlobalVars.self_obj.inventory
