...
func check_if_locking(map_player_list):
	...
	if all_players_in_room: MultiplayerTestenv.get_server().map_object_event(parent_map.get_map_server_id(), {"Scope": "Room", "Room ID": id, "Action": "Block Exits"})
...
func log_enemy_death(_dead_enemy):
	...
	if enemy_count <= 0: 
		room_cleared = true
		MultiplayerTestenv.get_server().map_object_event(parent_map.get_map_server_id(), {"Scope": "Room", "Room ID": id, "Action": "Unblock Exits"})
		
		if type == 'Exit Room':
			MultiplayerTestenv.get_server().map_object_event(parent_map.get_map_server_id(), {"Scope": "Map", "Action": "Victory"})
