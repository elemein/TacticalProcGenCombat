...
const MAP_GEN = preload("res://Scripts/Map/Generation/MapGenerator.gd")
...
func _init(d_name, flr_count):
	if [d_name, flr_count] == [null, null]:
		...
		self.set_name(self.dungeon_name)
		self.floor_count = flr_count

func _ready():
	if self.create_instance:
		for flr_no in range(1, self.floor_count+1):
			var floor_name = 'Dungeon Floor %d' % [flr_no]
			var map_type = 'Normal Floor' if flr_no < self.floor_count else 'End Floor'
			var new_floor = self.map_generator.generate(self, floor_name, flr_no, map_type)
...
func organize_child_map_nodes():
	for key in self.floors:
		self.floors[key].organize_map_floor()
...
func add_map_to_mapset(map):
	self.floors[map.get_map_name()] = map
	add_child(map)
