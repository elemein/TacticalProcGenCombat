...
func _ready():
	var animation_player = self.character.find_node('AnimationPlayer')
	animation_player.get_animation('idle').set_loop(true)
	animation_player.play('idle')
	
	for ability_button in self.ability_box.get_children():
		ability_button.connect('pressed', self, '_on_ability_selected')
	self.lock_in.connect("pressed", self, '_on_LockIn_pressed')
	self.left_turn.connect("pressed", self, '_on_Left_button_down')
	self.right_turn.connect("pressed", self, '_on_Right_button_down')
